    ...
    <Tabs groupId="operating-systems" queryString="os">
      <TabItem value="macos" label="MacOS">
        ...
        2. Configure the environment variables in the `docker-compose.yml`
           ...
           - For authentication, you can configure:
             - Magic links with `RESEND_API_KEY` for email-based authentication
             - Google OAuth with `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`
             - Microsoft Entra ID with `MICROSOFT_CLIENT_ID`, `MICROSOFT_CLIENT_SECRET`, and optionally `MICROSOFT_TENANT_ID` (defaults to "common" for multi-tenant applications)
