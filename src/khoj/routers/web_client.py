...
def login_page(request: Request):
    ...
    has_google = bool(os.environ.get("GOOGLE_CLIENT_ID") and os.environ.get("GOOGLE_CLIENT_SECRET"))
    has_microsoft = bool(os.environ.get("MICROSOFT_CLIENT_ID") and os.environ.get("MICROSOFT_CLIENT_SECRET"))
    has_magic_link = bool(os.environ.get("RESEND_API_KEY"))
    
    # Get redirect URIs for OAuth providers
    context = {"request": request, "next_url": next_url}
    
    # Add Google OAuth details if enabled
    if has_google:
        google_client_id = os.environ.get("GOOGLE_CLIENT_ID")
        google_redirect_uri = str(request.app.url_path_for("auth_post"))
        context.update({
            "google_enabled": True,
            "google_client_id": google_client_id,
            "google_redirect_uri": f"{google_redirect_uri}?next={next_url}",
        })
    else:
        context["google_enabled"] = False
        
    # Add Microsoft OAuth details if enabled
    if has_microsoft:
        ...
        context.update({
            ...
            "microsoft_redirect_uri": f"{microsoft_redirect_uri}?next={next_url}",
        })
    else:
        context["microsoft_enabled"] = False
        
    # Add magic link availability
    context["magic_link_enabled"] = has_magic_link
    
    # Check if any auth method is available
    context["any_auth_enabled"] = has_google or has_microsoft or has_magic_link
