...
export default function LoginPrompt(props: LoginPromptProps) {
	const { data, error, isLoading } = useSWR<CredentialsData>(
		"/auth/oauth/metadata",
		fetcher,
	);
		...
		fetch("/auth/magic", {
			...
			.then((res) => {
				if (res.ok) {
					...
					setSendEmailError(
						"Failed to send email. Contact developers for assistance.",
					);
		...
		<div className="flex flex-col gap-4 p-4">
			...
			{!checkEmail && (
				<div className="flex items-center justify-center gap-4 text-muted-foreground flex-col">
					...
					{sendEmailError && (
						<div className="text-red-500 text-sm">{sendEmailError}</div>
					)}
				</div>
			)}
			{checkEmail && (
				<div className="flex items-center justify-center gap-4 text-muted-foreground flex-col">
					...
					{otpError && (
						<div className="text-red-500 text-sm">
							{otpError} {ALLOWED_OTP_ATTEMPTS - numFailures} remaining
							attempts.
						</div>
					)}
				</div>
	...
	return (
		<div className="flex flex-col gap-4 p-4 md:p-0">
			...
			<div className="text-center text-muted-foreground text-sm mb-[20px]">
				...
				See <Link href="https://khoj.dev/privacy-policy">Privacy Policy</Link>.
			</div>
		</div>
	);
}
